...
class RankingService {
  static int calculateIdeaPopularity(Idea idea) {
    ...
    score += (idea.createdAt.isAfter(DateTime.now().subtract(Duration(days: 7))) ? 20 : 0);
    return score;
  }

  static int calculateThiefRespect(Thief thief) {
    int respect = 1;
  ...
  static int calculateRealizationScore(Realization realization) {
    ...
    score += (realization.createdAt.isAfter(DateTime.now().subtract(Duration(days: 3))) ? 10 : 0);
    return score;
  }
}
